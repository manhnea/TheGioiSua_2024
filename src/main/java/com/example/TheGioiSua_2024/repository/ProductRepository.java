...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query("SELECT new com.example.TheGioiSua_2024.dto.ProductDto( "
            ...
            + "WHERE p.status = 1 "
            + "GROUP BY p.id, p.productname, mt.id, mb.id, tt.id, mt.milkTypename, mb.milkbrandname, tt.targetName, p.productUrl, p.imgUrl, p.status "
            + "ORDER BY p.id DESC")
    List<ProductDto> getPageProduct();
    ...
    @Query("SELECT new com.example.TheGioiSua_2024.dto.ProductDto( "
            ...
            + "JOIN p.milkType mt "
            + "JOIN p.targetUser tt "
            + "JOIN Invoicedetail id ON id.milkDetail.id = md.id "
            + "JOIN Invoice inv ON id.invoice.id = inv.id "
            + "WHERE p.status = 1 AND inv.status = 913 "
            + "GROUP BY p.id, p.productname, mt.id, mb.id, tt.id, mt.milkTypename, mb.milkbrandname, tt.targetName, p.productUrl, p.imgUrl, p.status "
