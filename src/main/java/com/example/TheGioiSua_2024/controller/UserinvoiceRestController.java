...
public class UserinvoiceRestController {
  ...
  @PostMapping("/add")
  public ResponseEntity<?> addUserinvoice(@RequestBody @Valid Userinvoice userinvoice,
    ...
    return ResponseEntity.ok(
        Map.of("status", "success", "message", userinvoiceService.saveUserinvoice(userinvoice)));
  }

  //RessourceEndPoint:http://localhost:1234/api/Userinvoice/update
  @PutMapping("/update/{id}")
  public ResponseEntity<?> updateUserinvoice(@PathVariable Long id,
      @RequestBody @Valid Userinvoice userinvoice, BindingResult bindingResult) {
    if (bindingResult.hasErrors()) {
      List<Map<String, String>> errors = new ArrayList<>();
      for (FieldError fieldError : bindingResult.getFieldErrors()) {
        Map<String, String> error = new HashMap<>();
    ...
    return ResponseEntity.ok(Map.of("status", "success", "message",
        userinvoiceService.updateUserinvoice(id, userinvoice)));
  }

  //RessourceEndPoint:http://localhost:1234/api/Userinvoice/delete
  @DeleteMapping("/delete/{id}")
  public ResponseEntity<?> deleteUserinvoice(@PathVariable Long id,
      @RequestBody Userinvoice userinvoice) {
  ...
  public List<Map<String, Object>> getUserInvoices() {
    return userinvoiceService.getUserInvoices();
  }
}
